...
public class LS {
    ...
    public static double[][] ls(final double data[], final int rows, final int cols, int k, int n) {
        ...
        //double P[] = ArrayUtils.random(rows * k, eps, 1), Q[] = ArrayUtils.random(cols * k, eps, 1);
        ...
        for(int i = 0; i < data.length; i++) {
            if(data[i] !=0) {
                int r = ArrayUtils.idx2r(i, cols),
                c = ArrayUtils.idx2c(i, cols);
        ...
        for(int i = 0; i < n; i++) {
            ...
            for(Sample s : samples) {
                ...
                Multiplication.mult(P, Q, PQ, rows, cols, k);
                // double prediction = b + b_r[s.r] + b_c[s.c] + PQ[ArrayUtils.rc2idx(s.r, s.c, cols)]
                double prediction = PQ[ArrayUtils.rc2idx(s.r, s.c, cols)],
                e = (s.v - prediction);
